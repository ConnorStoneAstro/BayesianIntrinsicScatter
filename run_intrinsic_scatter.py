from generate_mock_data import X, X_err, Y, Y_err, sample_phi_params, sample_phi, generate_sample
...
def relation_f_fit(xx, yy):
    ...
    myodr = odr.ODR(mydata, odr.polynomial(1), beta0 = [1., 1.])
    res_m = myodr.run()
    return res_m.beta
...
if __name__ == '__main__':
    ...
    S, P = BayesianIntrinsicScatter(observed, total_scatter*1.1, X, Y, sample_phi_params, sample_phi, relation_f, relation_f_fit)

    # Perform classical intrinsic scatter analysis
    classical_sigma2avg = np.sqrt(np.mean(
        (params[1]*np.array(list(map(X_err, observed))))**2
        + np.array(list(map(Y_err, observed)))**2
    ))
    classical_int_scatter = np.sign(total_scatter**2 - classical_sigma2avg**2)*np.sqrt(np.abs(total_scatter**2 - classical_sigma2avg**2))
    ...
    plt.axvline(intrinsic_scatter, color = 'r', linewidth = 2, label = 'true $\\sigma_i = %.2f$' % intrinsic_scatter)
    plt.axvline(max(0,classical_int_scatter), color = 'k', linewidth = 2, linestyle = '--', label = 'classical $\\sigma_i = %.2f$' % classical_int_scatter)
    plt.legend()
